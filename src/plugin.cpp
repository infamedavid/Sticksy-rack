...
struct SticksyBlankModule : Module {
	...
	bool hasUnlockedStickers() const { for(const StickerEntry& e:stickers) if(!e.locked) return true; return false; }
	void shakeMultipleStickers(){ if(mode!=MODE_MULTIPLE||stickers.empty()) return; bool changed=false; for(StickerEntry& e:stickers){ if(e.locked) continue; assignPlacementForMultiple(e); changed=true; } if(changed) stickerVersion++; }
...
struct SticksyBlankWidget : ModuleWidget {
	...
	void appendContextMenu(Menu* menu) override {
		...
		struct StickerItem:MenuItem{ SticksyBlankModule* module; int index=-1; Menu* createChildMenu() override { Menu* submenu=new Menu; struct LockItem:MenuItem{ SticksyBlankModule* module; int index=-1; void onAction(const event::Action&) override { if(!module||index<0||index>=(int)module->stickers.size()) return; SticksyBlankModule* m=module; int lockIndex=index; pushSticksyModuleChange(module,"lock Sticksy sticker",[m,lockIndex](){ if(lockIndex<0||lockIndex>=(int)m->stickers.size()) return; m->stickers[lockIndex].locked=!m->stickers[lockIndex].locked; }); } void step() override { MenuItem::step(); rightText=(module&&index>=0&&index<(int)module->stickers.size()&&module->stickers[index].locked)?"✔":""; }}; auto* lock=createMenuItem<LockItem>("Lock"); lock->module=module; lock->index=index; lock->disabled=!module||index<0||index>=(int)module->stickers.size(); submenu->addChild(lock); struct DeleteItem:MenuItem{ SticksyBlankModule* module; int index=-1; void onAction(const event::Action&) override { if(!module||index<0||index>=(int)module->stickers.size()) return; SticksyBlankModule* m=module; int deleteIndex=index; pushSticksyModuleChange(module,"delete Sticksy sticker",[m,deleteIndex](){ if(deleteIndex<0||deleteIndex>=(int)m->stickers.size()) return; m->stickers.erase(m->stickers.begin()+deleteIndex); m->stickerVersion++;}); }}; auto* del=createMenuItem<DeleteItem>("Delete"); del->module=module; del->index=index; del->disabled=!module||index<0||index>=(int)module->stickers.size(); submenu->addChild(del); return submenu; }};
