...
struct SticksyBlankModule : Module {
	...
	std::string ensureLibraryCopyPath(const std::string& sourcePath) {
		...
		std::string filename=source.filename().u8string(), stem=source.stem().u8string(), ext=source.extension().u8string();
		std::filesystem::path libraryDir=std::filesystem::u8path(asset::user("Sticksy/stickers")); std::error_code ec; std::filesystem::create_directories(libraryDir,ec); if(ec) return sourcePath;
		std::filesystem::path candidate=libraryDir/filename; int suffix=1;
		while(std::filesystem::exists(candidate,ec)){ if(ec) return sourcePath; std::ostringstream name; name<<stem<<"_"<<std::setfill('0')<<std::setw(3)<<suffix++<<ext; candidate=libraryDir/name.str(); }
		std::filesystem::copy_file(source,candidate,std::filesystem::copy_options::none,ec); if(ec) return sourcePath; return candidate.u8string();
	}
	...
	math::Vec estimateStickerSize(const StickerEntry& entry) {
		...
		if(file.is_open()){
			...
			if(std::regex_search(text,m,re)&&m.size()>=5){
				try {
					float vw=std::stof(m[3].str()), vh=std::stof(m[4].str());
					if(!isBad(vw)&&!isBad(vh)) return math::Vec(vw,vh);
				}
				catch (...) {
				}
			}
	...
	void dataFromJson(json_t* rootJ) override {
		...
		clearStickers(); json_t* stickersJ=json_object_get(rootJ,"stickers"); if(stickersJ&&json_is_array(stickersJ)){ size_t n=json_array_size(stickersJ); for(size_t i=0;i<n;i++){ json_t* sj=json_array_get(stickersJ,i); if(!sj||!json_is_object(sj)) continue; json_t* pathJ=json_object_get(sj,"path"); if(!(pathJ&&json_is_string(pathJ))) continue; StickerEntry e; e.path=json_string_value(pathJ); e.displayName=system::getFilename(e.path); if(json_t* d=json_object_get(sj,"displayName"); d&&json_is_string(d)) e.displayName=json_string_value(d); if(json_t* x=json_object_get(sj,"x"); x&&json_is_number(x)) e.x=json_number_value(x); if(json_t* y=json_object_get(sj,"y"); y&&json_is_number(y)) e.y=json_number_value(y); if(json_t* r=json_object_get(sj,"rotation"); r&&json_is_number(r)) e.rotation=json_number_value(r); loadStickerSvg(e); if(mode==MODE_SINGLE){ replaceSingleSticker(e); break; } addMultipleSticker(e);} }
	}
...
struct SticksyBlankWidget : ModuleWidget {
	...
	void pushModuleChange(SticksyBlankModule* module,const std::string& name,std::function<void()> applyChange){ if(!module) return; auto* h=new history::ModuleChange; h->name=name; h->moduleId=module->id; h->oldModuleJ=module->toJson(); applyChange(); h->newModuleJ=module->toJson(); APP->history->push(h);} 
